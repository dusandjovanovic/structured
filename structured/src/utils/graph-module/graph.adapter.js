...
export const graphAdapterNodeRemove = (node, graph) => {
    ...
    graph.nodes = graph.nodes.filter((element) =>
        (element.key !== node));
};

export const graphAdapterEdgeRemove = (source, target, graph) => {
    graph.edges = graph.edges.filter((element) =>
        (element.source.key !== source || element.target.key !== target));
    graph.nodes.map(node => {
        if (node.key === target)
           node.inEdges = node.inEdges.filter((element) =>
            element !== source);
        else if (node.key === source)
           node.outEdges = node.outEdges.filter((element) =>
            element !== target);
       return node;
    });
};